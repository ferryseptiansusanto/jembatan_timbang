...
from modules.config.config import BERAT_MASK_TEMPLATE  # ambil mask dari config kamu
from modules.helper.fontsetup import SetupEdLineFont, apply_thousand_separator
...
from modules.helper.db import get_nama_barang_aktif, get_nama_pemasok_pelanggan_aktif, \
    ...
    update_transaksi_timbang_kedua, SmartTableModel, fetch_transaksi, count_transaksi
...
class TimbangMain(QtWidgets.QWidget, Ui_Form):
    ...
    def __init__(self, current_user, current_user_level):
        ...
        self.parser = SerialParser(BERAT_MASK_TEMPLATE)
    ...
    def update_weight_display(self, data):
        ...
        if not self.parser.validate(data):
            print("[IGNORED] Format tidak sesuai mask")
            return
        ...
        if weight is None:
            print("[ERROR] Berat tidak bisa diambil")
            return

        formatted = "{:,.1f}".format(weight).replace(",", ".")  # 12.345,0
        self.edCurrentWeight.setText(f"{formatted} {unit}")
