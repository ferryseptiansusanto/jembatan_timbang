...
from .form_pelanggan import Ui_Form  # pastikan form_user.py ada di folder yang sama
...
class PelangganMain(QtWidgets.QWidget, Ui_Form):
    def __init__(self, current_user, current_user_level):
        ...
        self.setWindowTitle(f"Master Kustomer - Login oleh: {self.current_user}")

    def setup_model(self):
        self.model = QSqlTableModel()
        self.model = CheckBoxSqlTableModel(checkbox_column=7)
        self.model.setTable(TABLES["TblPelanggan"])
        self.model.select()
        self.model.setHeaderData(1, QtCore.Qt.Horizontal, "Nama Kustomer")
    ...
    def add_record(self):
        ...
        try:
            execute_query(f"""
                INSERT INTO {TABLES['TblPelanggan']} (nama, alamat, created_at, modified_at, created_by
                , modified_by, active)
                VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    def delete_record(self):
        ...
        if confirm == QtWidgets.QMessageBox.Yes:
            execute_query(f"DELETE FROM {TABLES['TblPelanggan']} WHERE id = ?", (self.id_record,))
